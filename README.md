...
### 2. 평가 단계
```
statement()에서 AST 구축 완료
  → 파싱 성공 여부 판단 (구문 에러만 체크)
    → 파싱 성공 시 (OK) 출력
      → 참조된 변수의 정의 여부 확인
        → 모든 변수가 정의됨: expr_tree->evaluate() 실행
        → 정의되지 않은 변수 존재: 평가 생략, Unknown 처리
```
...
#### 1. 구문 에러 (Syntax Error) - 파싱 단계
다음 에러는 **(ERROR)** 메시지와 함께 출력됨:
- 식별자 누락
- 대입 연산자(:=) 누락
- 괄호 불일치
- **중복 연산자** (예: `+ +`, `* *`)

#### 2. 의미 에러 (Semantic Error) - 평가 단계
다음 에러는 **(OK)** 출력 후 평가만 실패:
- **정의되지 않은 변수 참조**
  - 구문 분석은 성공 → (OK) 출력
  - 평가는 실패 → Result에서 `Unknown` 표시

### 에러 복구
- 에러 발생 시에도 파싱 계속 진행
- 구문 에러 발생 변수: 정의되지 않음 처리
- 평가 실패 변수: `Unknown`으로 표시

### 핵심 설계 원칙
**파싱(Parsing)과 평가(Evaluation)의 분리**
- 파싱: 문법적 정확성만 검증
- 평가: 의미적 정확성 검증 및 실제 계산
...
### 예제 4: 구문 에러와 의미 에러 혼합 (eval4.txt)
...
#### 분석
1. `operand1 = 3` → 정상
2. `operand2 := operand1 + + 2` → **구문 에러** (중복 연산자)
   - (ERROR) 출력
   - operand2는 정의되지 않음
3. `target := operand1 + operand2 * 3` → **의미 에러** (정의되지 않은 변수)
   - 구문은 올바름 → **(OK) 출력**
   - operand2가 정의되지 않아 평가 불가 → `Unknown`
